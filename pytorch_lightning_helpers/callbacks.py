...
class OutlierDetector(Callback):
    def __init__(self, dirpath, log_key, start_log=10000, stats_momentum=0.992,
                 score_momentum=0.99, sd_threshold=4):
        super().__init__()
    ...
    def get_record(self, loss_name, z_score, data_key):
        if data_key in self.outlier_logs[loss_name]:
            ...
            average_z_score = self.moving_average(prev_z_score, z_score,
                                                  self.score_momentum)
        else:
            count = 1
    ...
    def update_stats(self, loss_name, loss):
        ...
        self.loss_mean_logs[loss_name] = self.moving_average(self.loss_mean_logs[loss_name], mean, self.stats_momentum)
        self.loss_sd_logs[loss_name] = self.moving_average(self.loss_sd_logs[loss_name], sd, self.stats_momentum)
    ...
    def flush_logs(self):
        for loss_name, outliers in self.outlier_logs.items():
            ...
            with open(fpath, 'w', encoding='utf8', newline='') as output_file:
                ...
                rows = sorted(rows, key=lambda row: row['outlier_score'],
                              reverse=True)
