...
def _list_connected_datapipes(scan_obj, only_datapipe, cache):
    ...
    datapipe_classes: Tuple[Type[DataPipe]] = (IterDataPipe,)  # type: ignore[assignment]
...
def _traverse_helper(datapipe, only_datapipe, cache):
    if not isinstance(datapipe, (IterDataPipe,)):
        raise RuntimeError(
            "Expected `IterDataPipe` or `MapDataPipe`, but {} is found".format(
                type(datapipe)
            )
        )
