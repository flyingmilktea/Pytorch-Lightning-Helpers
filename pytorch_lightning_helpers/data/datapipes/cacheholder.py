...
class InMemoryCacheHolderIterDataPipe(IterDataPipe):
    ...
    def __init__(self, source_dp: IterDataPipe, size: Optional[int] = None) -> None:
        self.source_dp: IterDataPipe = source_dp
        # cache size in MB
        if size is not None:
...
def _promise_filename(filename):
    return filename.with_name(filename.name + ".promise")
...
class OnDiskCacheHolderIterDataPipe(IterDataPipe):
    ...
    def _cache_check_fn(data, filepath_fn, hash_dict, hash_type, extra_check_fn):
        filepaths = data if filepath_fn is None else supply_kwargs(filepath_fn, data)
        result = True
